...
No radio hardware on hand yet, but Track B-GPS is now fully done: a real u-blox 7 GPS unit got an outdoor 3D fix, and `GpsClock`'s offset-computation math has been validated end-to-end through a real `gpsd` instance (not just raw NMEA, not just the stub).
...
The remaining work before live over-the-air spots is the `deploy.sh`/`install.sh` dry run against that Pi and the RTL-SDR + HF antenna itself.
